...
module.exports=function(app, prefix) {
  ...
  app.get(prefix + '/:model', (req, res) => {
    ...
    console_wrapper.log('admin::list model', model);
    switch(model) {
      case 'channels':
  ...
  app.get(prefix + '/:model/:id', (req, res) => {
    ...
    console_wrapper.log('admin::findOne model', model, 'id', id);
    switch(model) {
      case 'users':
  ...
  app.get(prefix + '/channels/:cid/interactions', (req, res) => {
    ...
    //console_wrapper.log('loading channel', cid);
    cache.getChannelDeletions(cid, req.apiParams, function(interactions, err, meta) {
      if (err) {
        console_wrapper.error('getMessage err', err);
        const resObj={
          meta: {
            code: 500,
            error_message: err
  ...
  app.get(prefix + '/channels/:cid/messages/:mid', (req, res) => {
    ...
    //console_wrapper.log('message id', mid);
  ...
  app.post(prefix + '/:model', (req, res) => {
    const model = req.params.model;
    console_wrapper.log('admin::create model', model);
    switch(model) {
      case 'users':
  ...
  app.patch(prefix + '/:model/:id', (req, res) => {
    const model = req.params.model;
    const id = req.params.id;
    console_wrapper.log('admin::update model', model, 'id', id);
    switch(model) {
      case 'channels':
        cache.getChannel(id, req.apiParams, function(channel, err, meta) {
          // FIXME: WRITE ME
