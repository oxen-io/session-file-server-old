...
memoryUpdate = function (model, filter, data, callback) {
  ...
  var mem = this
  //console_wrapper.log('memoryUpdate - model', model, 'filter', filter, 'data', data, 'callback', callback)
  ...
  if (filter.id) {
    ...
    //console_wrapper.log('memoryUpdate - not implemented, search by?', filter, data)
    this.all(model, filter, function(err, nodes) {
      //console_wrapper.log('memoryUpdate - records', nodes)
...
function start(nconf) {
  ...
  //console_wrapper.log('default type', defaultSchemaType)
  ...
  //console_wrapper.log('configuring data', configData)
  ...
  //console_wrapper.log('configuring token', configData)
  ...
  if (schemaDataType==='mysql') {
    //console_wrapper.log('MySQL is active');
    ...
    schemaData.client.changeUser({ charset: 'utf8mb4' }, function(err) {
      if (err) console_wrapper.error('Couldnt set UTF8mb4', err);
      //console_wrapper.log('Set charset to utf8mb4 on Data');
    });
    schemaToken.client.changeUser({ charset: 'utf8mb4' }, function(err) {
      if (err) console_wrapper.error('Couldnt set UTF8mb4', err);
      //console_wrapper.log('Set charset to utf8mb4 on Token');
    });
  ...
  setInterval(function() {
    if (module.exports.connection) {
      module.exports.connection.ping(function (err) {
        if (err) {
          console_wrapper.log('lets_parser::monitor - reconnecting, no ping');
          ref.conn(module.exports.last.host, module.exports.last.user,
            module.exports.last.pass, module.exports.last.db);
        }
        //console_wrapper.log(Date.now(), 'MySQL responded to ping');
      })
    }
  }, 60000);
  */
  ...
  var statusmonitor=function () {
    if (schemaDataType=='mysql') {
      schemaData.client.ping(function (err) {
        if (err) {
          console_wrapper.log('trying to reconnect to data db');
    ...
    if (schemaDataType=='mysql') {
      schemaToken.client.ping(function (err) {
        if (err) {
          console_wrapper.log('trying to reconnect to token db');
        ...
        annotationModel.count({}, function(err, annotationCount) {
          ...
          if (schemaDataType=='sqlite3') {
            schemaData.client.get('PRAGMA page_count;', function(err, crow) {
              //console_wrapper.log('dataaccess.caminte.js::status sqlite3 page_count', row);
              schemaData.client.get('PRAGMA page_size;', function(err, srow) {
                var cnt=crow['page_count'];
                var psize=srow['page_size'];
                var size=cnt*psize;
                console_wrapper.log('dataaccess.caminte.js::status sqlite3 data [',cnt,'x',psize,'] size: ', size);
              });
            });
          }
          if (schemaDataType=='redis') {
            //console_wrapper.dir(schemaAuth.client.server_info);
            ...
            console_wrapper.log("dataaccess.caminte.js::status redis token "+schemaToken.client.server_info.used_memory_human+" "+schemaToken.client.server_info.db0);
            console_wrapper.log("dataaccess.caminte.js::status redis data "+schemaData.client.server_info.used_memory_human+" "+schemaData.client.server_info.db0);
          }
          console_wrapper.log('dataaccess.caminte.js::status '+userCount+'U '+annotationCount+'a '+fileCount+'f');
        });
      });
    });
  }
...
function db_insert(rec, model, callback) {
  console_wrapper.log('dataaccess.caminte.js::db_insert - start', rec, JSON.stringify(rec));
  rec.isValid(function(valid) {
    //console_wrapper.log('dataaccess.caminte.js::db_insert - Checked');
    if (valid) {
      //console_wrapper.log(typeof(model)+'trying to create ',rec);
      // mysql can't handle any undefineds tbh
      //console_wrapper.log('dataaccess.caminte.js::db_insert - Valid', rec, typeof(model));
      // sometimes model.create doesn't return...
      // maybe a timer to detect this (timeout) and callback
      model.create(rec, function(err) {
        //console_wrapper.log('dataaccess.caminte.js::db_insert - created');
        if (err) {
          console_wrapper.log(typeof(model)+" insert Error ", err);
        }
        if (callback) {
          //console_wrapper.log('dataaccess.caminte.js::db_insert - callingback');
          if (rec.id) {
      ...
      console_wrapper.log(typeof(model)+" validation failure");
      console_wrapper.dir(rec.errors);
      if (callback) {
...
function db_delete(id, model, callback) {
  model.destroyById(id, function(err, rec) {
    if (err) {
      console_wrapper.log("delUser Error ", err);
    }
    if (callback) {
      callback(rec, err);
    }
...
function db_get(id, model, callback) {
  id = parseInt(id);
  if (isNaN(id)) {
    console_wrapper.log('dataaccess.camtine.js::db_get - id isnt number', id);
    var stack = new Error().stack
    console_wrapper.log(stack);
    callback([], "id is not a number");
    return;
  }
  model.findById(id, function(err, rec) {
    if (err) {
      console_wrapper.log("db_get Error ", err);
    }
...
function applyParams(query, params, callback) {
  //console_wrapper.log('applyParams - params', params);
  /*
  var idfield='id';
  ...
  */
  ...
  query.model.find({ where: where, order: idfield+' DESC', limit: 1}, function(err, items) {
    ...
    */
    //console_wrapper.log('applyParams - maxid', maxid);
...
function setparams(query, params, maxid, callback) {
  ...
  //console_wrapper.log('into setparams from',params.since_id,'to',params.before_id,'for',params.count);
  ...
  */
  ...
  //console_wrapper.log('test',query.model.modelName);
  ...
  //if (query.debug) {
    //console_wrapper.log('dataaccess.caminte::setparams - model', query.model.modelName)
  //}
  if (query.model.modelName==='post' || query.model.modelName==='message') {
    //if (query.debug) {
      //console_wrapper.log('dataaccess.caminte::setparams - params', params.generalParams)
    //}
    // Remember this defaults to show deleted
    if (!params.generalParams || !params.generalParams.deleted) {
      //console_wrapper.log('hiding deleted')
  ...
  //console_wrapper.log('setparams query order', query.q.params);
  //console_wrapper.log('setparams params', params);
  var count = 20;
  if (params.pageParams) {
    ...
    console_wrapper.log('dataaccess.caminte::setparams - WARNING no pageParams in params');
  }

  // items are always returned from newest to oldest even if count is negative
  if (!query.q.params.order) {
    //console_wrapper.log('setparams count', count);
    //console_wrapper.log('setparams params.count', params.count);
    //if (count>0) {
      //console_wrapper.log('setparams sorting', idfield, 'desc');
      query=query.order(idfield, 'DESC')
    //}
    //if (count<0) {
      //console_wrapper.log('setparams sorting', idfield, 'asc');
  ...
  //console_wrapper.log('count', count, 'queryCount', queryCount);
  ...
  */
  //console_wrapper.log(query.model.modelName, 'from', params.since_id, 'to', params.before_id, 'should be a count of', params.count, 'test', params.before_id-params.since_id);
  ...
  */
  if (query.debug) {
    console_wrapper.log('dataaccess.caminte.js::setparams query', query.q);
  }
  var min_id=Number.MAX_SAFE_INTEGER, max_id=0;
  query.run({},function(err, objects) {
    if (err) {
      console_wrapper.error('dataaccess.caminte.js::setparams - err', err);
    }
    //console_wrapper.log('dataaccess.caminte.js::setparams -', query.model.modelName, 'query got', objects.length, 'only need', params.count);
    ...
    //console_wrapper.log('dataaccess.caminte.js::setparams - got', objects.length, 'queried', queryCount, 'asked_for', count);
    ...
    if (more) {
      ...
      if (count<0) { // id asc
        for(var i in objects) {
          console_wrapper.log('dataaccess.caminte.js::setparams - negative count', objects[i].id);
        }
        objects.pop();
      }
      */
    }
    //console_wrapper.log('dataaccess.caminte.js::setparams - resultset got', objects.length, 'range:', min_id, 'to', max_id, 'more:', more);
    // generate meta, find min/max in set
    for(var i in objects) {
      var obj=objects[i];
      //console_wrapper.log('dataaccess.caminte.js::setparams - idx', obj[idfield], 'min', min_id, 'max', max_id);
      if (obj[idfield]) {
...
module.exports = {
  ...
  addUser: function(username, password, callback) {
    userModel.create({
        ...
        if (err) {
          console_wrapper.log('dataaccess.caminte.js::addUser - create err', err);
        }
        if (callback) {
          callback(user, err);
        }
  ...
  setUser: function(iuser, ts, callback) {
    ...
    //console_wrapper.log('camtinejs::setUser - iuser', iuser);
    // doesn't overwrite all fields
    userModel.findOne({ where: { id: iuser.id } }, function(err, user) {
      //console_wrapper.log('camtinejs::setUser - got res', user);
      if (user) {
        //console_wrapper.log('camtinejs::setUser - updating user', user.id);
        //console_wrapper.log('camtinejs::setUser - updating data', iuser);
        ...
        */
        ...
        //iuser.created_at =
        //console_wrapper.log('camtinejs::setUser - final data', iuser);
        ...
        userModel.update({ where: { id: iuser.id } }, iuser, function(err, userRes) {
          if (err) {
            console_wrapper.error('camtinejs::setUser - ', err);
          }
          // userRes is the number of updated rows I think
          if (callback) callback(user, err);
        });
      } else {
        //console_wrapper.log('camtinejs::setUser - creating user');
  ...
  patchUser: function(userid, changes, callback) {
    ...
    userModel.update({ where: { id: userid } }, changes, function(err, user) {
      if (err) console_wrapper.error('dataaccess.caminte.js::patchUser - err', err);
      // console_wrapper.log('patchUser user changes', user);
      if (callback) {
        callback(user, err);
      }
    });
  ...
  getUserID: function(username, callback) {
    if (!username) {
      console_wrapper.log('dataaccess.caminte.js::getUserID() - username was not set');
    ...
    //console_wrapper.log('dataaccess.caminte.js::getUserID(', username, ') - start');
    var ref=this;
  ...
  getUser: function(userid, callback) {
    if (userid == undefined) {
      console_wrapper.trace('dataaccess.caminte.js:getUser - userid is undefined');
      callback(null, 'dataaccess.caminte.js:getUser - userid is undefined');
      return;
    }
    if (!userid) {
      console_wrapper.log('dataaccess.caminte.js:getUser - userid isn\'t set');
      callback(null, 'dataaccess.caminte.js:getUser - userid isn\'t set');
      return;
    }
    if (callback == undefined) {
      console_wrapper.trace('dataaccess.caminte.js:getUser - callback is undefined');
      callback(null, 'dataaccess.caminte.js:getUser - callback is undefined');
      return;
    }
    //console_wrapper.log('dataaccess.caminte.js:getUser - userid', userid);
    if (userid[0]==='@') {
      //console_wrapper.log('dataaccess.caminte.js:getUser - getting by username');
      this.getUserID(userid.substr(1), callback);
      return;
    }
    if (isNaN(userid)) {
      console_wrapper.log('dataaccess.caminte.js:getUser - userid isn\'t a number');
      var stack = new Error().stack
      console_wrapper.error(stack)
      callback(null, 'dataaccess.caminte.js:getUser - userid isn\'t a number');
      return;
    }
    var ref=this;
    //console_wrapper.log('dataaccess.caminte.js:getUser - getting', userid);
    db_get(userid, userModel, function(user, err) {
      //console_wrapper.log('dataaccess.caminte.js:getUser - got', user);
      if (user==null && err==null) {
        if (ref.next) {
  ...
  getUsers: function(userids, params, callback) {
    if (!userids.length) {
      console_wrapper.log('dataaccess.caminte::getUsers - no userids passed in');
  ...
  searchUsers: function(query, params, callback) {
    ...
    function setDone(sys) {
      ...
      //console_wrapper.log('searchUsers', sys, ids.length);
      done[sys]=true;
      for(var i in done) {
        if (!done[i]) {
          //console_wrapper.log('searchUsers -', i, 'is not done');
          return;
        }
      }
      console_wrapper.log('searchUsers done', ids.length);
      if (!ids.length) {
  ...
  getAppCallbacks: function(client_id, client_secret, callback) {
    if (client_id===undefined) {
      console_wrapper.log('dataaccess.caminte::getAppCallbacks - no client_id passed in')
    ...
    if (!client_secret) {
      ...
      oauthAppModel.findOne({ where: { client_id: client_id } }, function(err, oauthApp) {
        if (err || !oauthApp) {
          console_wrapper.log('getAppCallbacks - err', err)
    ...
    oauthAppModel.findOne({ where: { client_id: client_id, secret: client_secret } }, function(err, oauthApp) {
      if (err || !oauthApp) {
        if (err) console_wrapper.log('getAppCallbacks - err', err)
        callback(null, 'err or app not found');
        return;
      }
      oauthCallbackModel.find({ where: { appid: oauthApp.id } }, function(err, callbacks) {
  ...
  createSession: function(client_id, redirect_uri, response_type, requested_scopes, state, callback) {
    console_wrapper.log('state', state, 'requested_scopes', requested_scopes);
    ...
    sessionModel.create({
      ...
      if (err) {
        console_wrapper.log('createSession - err', err);
      }
      //console_wrapper.log('createSession - obj', obj);
      callback(obj);
    })
  },
  authSession: function(code, userid, username, upstream_token, localToken, callback) {
    sessionModel.find({ where: { code: code } }, function(err, sessions) {
      if (err) {
        console_wrapper.log('authSession - err', err);
      }
      if (sessions) {
        if (sessions.length == 1) {
          ...
          ses.save(function(uErr) {
            if (uErr) {
              console_wrapper.log('authSession - uErr', uErr);
            }
            callback(ses, uErr);
          });
        } else {
          console_wrapper.log('authSession - too many sessions for that code');
          callback({}, 'too many sessions for that code');
          return;
        }
      } else {
        console_wrapper.log('authSession - no sessions for that code');
      ...
      if (tokenUnique==null) {
        ...
        localUserTokenModel.findOne({ where: { userid: userid, client_id: client_id }}, function(err, usertoken) {
          if (usertoken==null) {
            ...
            //console_wrapper.log('creating localUserToken', usertoken)
            ...
            console_wrapper.log('Already have token');
            //usertoken.userid=userid;
            //usertoken.client_id=client_id;
        ...
        //console_wrapper.log('already had token on file', tokenUnique);
        //console_wrapper.log('compare against', userid, client_id);
        // probably should check scopes
        if (userid==tokenUnique.userid && client_id==tokenUnique.client_id) {
          ...
          console_wrapper.log('already had token on file', tokenUnique);
          console_wrapper.log('compare against', userid, client_id);
          console_wrapper.log('tests', userid==tokenUnique.userid, client_id==tokenUnique.client_id);
  ...
  createOrFindUserToken: function(userid, client_id, scopes, callback) {
    //console_wrapper.log('createOrFindUserToken', userid, client_id, scopes)
    if (scopes===undefined) scopes='';
    localUserTokenModel.findOne({ where: { userid: userid, client_id: client_id }}, function(err, usertoken) {
      if (usertoken) {
        //console_wrapper.log('createOrFindUserToken found token', usertoken)
      ...
      genCheckToken(function(token) {
        ...
        usertoken.scopes=scopes;
        usertoken.token=token;
        //console_wrapper.log('dataaccess.caminte.js::createOrFindUserToken - creating localUserToken', usertoken)
        /*usertoken.save(function() {
          callback(usertoken, null);
        })*/
        //console_wrapper.log('createOrFindUserToken made token', usertoken)
        // this will call callback if set
        db_insert(usertoken, localUserTokenModel, callback);
      })
    })
  ...
  getAPITokenByUsername: function(username, callback) {
    //console_wrapper.log('dataaccess.camintejs.js::getAPITokenByUsername - username:', username);
    if (username==undefined) {
      console_wrapper.log('dataaccess.camintejs.js::getAPITokenByUsername - username not defined');
    ...
    this.getUserID(username, function(user, err) {
      ...
      localUserTokenModel.findOne({ where: { userid: user.id }, limit: 1 }, function(err, usertoken) {
        if (err) {
          console_wrapper.log('dataaccess.camintejs.js::getAPIUserToken - err', err, 'usertoken', usertoken);
        }
        //console_wrapper.log('dataaccess.camintejs.js::getAPIUserToken - found:', usertoken);
        callback(usertoken, err);
      });
    });
  },
  getAPIUserToken: function(token, callback) {
    //console_wrapper.log('dataaccess.camintejs.js::getAPIUserToken - Token:', token);
    if (token==undefined) {
      //console_wrapper.log('dataaccess.camintejs.js::getAPIUserToken - Token not defined');
...
*/
  ...
  setUpstreamUserToken: function(userid, token, scopes, callback) {
    upstreamUserTokenModel.findOne({ where: { userid: userid } }, function(err, upstreamToken) {
      if (err) {
        console_wrapper.log('dataaccess.camintejs.js::setUpstreamUserToken - upstreamUserTokenModel err', err);
        if (callback) {
      ...
      if (upstreamToken) {
        if (upstreamToken.token!=token) {
          console_wrapper.log('dataaccess.camintejs.js::setUpstreamUserToken - new token?', token, 'old', upstreamToken.token);
        }
      } else {
  ...
  delUpstreamUserToken: function(token) {
    console_wrapper.log('dataaccess.camintejs.js::delUpstreamUserToken - write me!');
  },
  ...
  addAPIAppToken: function(client_id, token, request) {
    console_wrapper.log('api_persistent_storage::addAPIAppToken - write me!');
  },
  delAPIAppToken: function(client_id, token) {
    console_wrapper.log('api_persistent_storage::delAPIAppToken - write me!');
  },
  getAPIAppToken: function(client_id, token) {
    console_wrapper.log('api_persistent_storage::getAPIAppToken - write me!');
  },
  /* client upstream token */
  addUpstreamClientToken: function(token, scopes) {
    console_wrapper.log('api_persistent_storage::addUpstreamClientToken - write me!');
  },
  delUpstreamClientToken: function(token) {
    console_wrapper.log('api_persistent_storage::delUpstreamClientToken - write me!');
  },
  getUpstreamClientToken: function() {
    console_wrapper.log('api_persistent_storage::getUpstreamClientToken - write me!');
  },
  /** user stream */
  findOrCreateUserStream: function(connectionId, tokenId, userId, autoDelete, callback) {
    //console_wrapper.log('dataaccess.camintejs::findOrCreateUserStream - start', connectionId, tokenId);
    //console_wrapper.log('connectionId', connectionId)
    //console_wrapper.log('tokenId', tokenId)
    //console_wrapper.log('userId', userId)
    //console_wrapper.log('autoDelete', autoDelete)
    // I don't think this is working
    userStreamModel.find({ where: { connection_id: connectionId, tokenid: tokenId } }, function(err, streams) {
      if (err) {
        console_wrapper.log('dataaccess.camintejs::findOrCreateUserStream - err', err);
      }
      if (!streams.length) {
        // create one
        ...
        userStreamModel.create(stream, function(err, createdStream) {
          //console_wrapper.log('dataaccess.camintejs::findOrCreateUserStream - created', createdStream);
        ...
        if (streams.length === 1) {
          ...
          if (stream.userid == userId) {
            ...
            //console_wrapper.log('dataaccess.camintejs::findOrCreateUserStream - found', stream);
            callback(stream, err);
            //})
          } else {
            console_wrapper.error('dataaccess.camintejs::findOrCreateUserStream - userid', userId, 'tried to change', stream.userid, 'stream', connectionId);
            callback([], 'not your stream');
          }
        } else {
          console_wrapper.error('dataaccess.camintejs::findOrCreateUserStream - too many, connectionId:', connectionId, 'tokenId', tokenId);
          callback([], 'too many');
        }
      }
    })
  ...
  findOrCreateUserSubscription: function(connectionNumId, stream, params, callback) {
    console_wrapper.log('dataaccess.camintejs::findOrCreateUserSubscription', connectionNumId, stream)
    if (connectionNumId === undefined) {
      console_wrapper.log('dataaccess.camintejs::findOrCreateUserSubscription - connectionNumId is empty', connectionNumId, stream)
    ...
    userStreamSubscriptionModel.find({ where: { user_stream_id: connectionNumId, stream: { like: stream } } }, function(err, subscriptions) {
      // subscription
      if (err) {
        console_wrapper.log('dataaccess.camintejs::findOrCreateUserSubscription - err', err);
      }
      if (subscriptions.length) {
        if (subscriptions.length === 1) {
          ...
          // too many
          console_wrapper.error('dataaccess.camintejs::findOrCreateUserSubscription - too many', subscriptions);
          callback({}, 'too many');
        }
      } else {
        userStreamSubscriptionModel.create({ user_stream_id: connectionNumId, stream: stream, params: params }, function(createErr, createdSub) {
          //console_wrapper.log('dataaccess.camintejs::findOrCreateUserSubscription - created', createdSub);
    ...
    userStreamSubscriptionModel.findOrCreate({
      ...
      console_wrapper.log('dataaccess.camintejs::findOrCreateUserSubscription - findOrCreate', subscription)
  ...
  deleteUserStream: function(connectionNumId, callback) {
    //console_wrapper.log('deleteUserStream', connectionNumId);
  ...
  getAllUserStreams: function(callback) {
    //console_wrapper.log('dataaccess.caminte.js::getAllUserStreams - start');
    userStreamModel.all(function(err, userStreams) {
      //console_wrapper.log('dataaccess.caminte.js::getAllUserStreams - result', userStreams.length);
      var ids = []
      var tokens = []
      ...
      //console_wrapper.log('dataaccess.caminte.js::getAllUserStreams - tokens', tokens.length, 'ids', ids.length)
      var done = {
        subs: false,
        tokens: false,
      }
      function doneCheck(type, data) {
        //console_wrapper.log('dataaccess.caminte.js::getAllUserStreams:doneCheck - ', type, data.length)
        done[type] = data;
        var complete = true;
        for(var i in done) {
          if (done[i] === false) {
  ...
  addFile: function(file, token, callback) {
    ...
    if (this.next) {
      ...
      //console_wrapper.log('Checking', file.urlexpires, typeof(file.urlexpires))
      ...
      console_wrapper.log('final pre file model', file);
      ...
      db_insert(new fileModel(file), fileModel, function(rec, err) {
        //console_wrapper.log('camintejs::addPost - res', rec, err);
        // process id
        /*
        ...
        */
        ...
        console_wrapper.log('camintejs::addFile - final', rec);
        // set thread_id
        callback(rec, err);
      });
    }
  ...
  getFile: function(fileId, callback) {
    //console_wrapper.log('dataaccess.caminte.js::getFile - id is '+id);
    if (fileId==undefined) {
    ...
    db_get(fileId, fileModel, function(file, err) {
      //console_wrapper.log('dataaccess.caminte.js::getFile - post, err',post,err);
      if (file==null && err==null) {
        //console_wrapper.log('dataaccess.caminte.js::getFile - next?',ref.next);
        if (ref.next) {
          //console_wrapper.log('dataaccess.caminte.js::getFile - next');
  ...
  addAnnotation: function(idtype, id, type, value, callback) {
    console_wrapper.log('addAnnotation idtype', idtype, 'id', id, 'type', type, 'value', value);
  ...
  clearAnnotations: function(idtype, id, callback) {
    //console_wrapper.log('dataaccess.caminte.js::clearAnnotations - idtype', idtype, 'id', id)
      ...
      for(var i in oldAnnotations) {
        ...
        if (oldNote.id) {
          annotationModel.destroyById(oldNote.id, function(err) {
            if (err) {
              console_wrapper.log('couldn\'t destory old annotation ', err);
            }
          });
        } else {
          //console_wrapper.log('dataaccess.caminte.js::clearAnnotations - OldNote doesn\'t have id ',oldNote);
        }
      }
      if (callback) {
        callback();
  ...
  updateUserCounts: function(userid, callback) {
    ...
    userModel.findById(userid, function(err, user) {
      if (!user) {
        console_wrapper.log('updateUserCounts no user', user, 'for id', userid);
        if (callback) {
          callback();
        }
        return;
      ...
      interactionModel.count({ where: { userid: userid, type: 'star' } }, function(err, starCount) {
        if (err) console_wrapper.error('updateUserCounts - star:', err);
    ...
    if (this.next) {
      ...
      if (alpha) {
        ...
        if (url.match(/\/photo\//)) {
          ...
          console_wrapper.log('dataaccess.camtine.js::getOEmbed -photo mode', postid, photoid)
          this.getAnnotations('post', postid, function(notes, err) {
            //console_wrapper.log('post info', notes);
            var c = 0
            for(var i in notes) {
              var note = notes[i]
              if (note.type == 'net.app.core.oembed') {
                console_wrapper.log('dataaccess.camtine.js::getOEmbed - found our info', note.value)
                c ++
                if (c == photoid) {
        ...
        console_wrapper.log('dataaccess.camtine.js::getOEmbed - postid', postid);
        this.getPost(postid, function(post, err) {
          //console_wrapper.log('post info', post);
          info.data = {
      ...
      //callback(null, null)
      //console_wrapper.log('dataaccess.caminte.js::getOEmbed - write me!');
      ...
      request(url, function(err, resp, body) {
        ...
        if (links) {
          for(var i=0,imax=links.length; i<imax; i++) {
            var link = links[i]
            //console_wrapper.log('link', link)
            if (link.match(/type=['"]application\/json\+oembed['"]/i)) {
              //console_wrapper.log('oembed link', link)
              if (link.match(/rel=['"]alternate["']/i)) {
                //console_wrapper.log('alt oembed link', link)
                var href = link.match(/href=["']([^"']+)["']/i)
                var oembedUrl = href[1]
                console_wrapper.log('href', oembedUrl)
      ...
      */
      ...
      oembedTools.extract(url).then((data) => {
        console_wrapper.log('url', url, 'data', data)
        callback(data, null);
      }).catch((err) => {
        console_wrapper.log('oembed err', err);
